...
template<typename IFooAPI>
void Utilities<typename IFooAPI>::AssassinDefaultAttack(int rank = 0)//rank是要vector中要攻击的序号，当vector中仅有一个时，自动选择
{
	int stux = API.GetStudents()[rank]->x;
	int stuy = API.GetStudents()[rank]->y;
	int sx = API.GetSelfInfo()->x;
	int sy = API.GetSelfInfo()->y;
	Attack(atan2(stuy - sy, stux - sx));
}

template<typename IFooAPI>
bool Utilities<typename IFooAPI>::AssassinDefaultAttackOver(int rank = 0)
{
	int stux = API.GetStudents()[rank]->x;
	int stuy = API.GetStudents()[rank]->y;
	int sx = API.GetSelfInfo()->x;
