...
{
    Node ret;
    ret.x = api.GetSelfInfo()->x;
    ret.y = api.GetSelfInfo()->y;
    ret.fCost = FLT_MAX;
    ret.gCost = FLT_MAX;
    ret.hCost = FLT_MAX;
    ret.parentX = -1;
...
{
    int tx = UsablePath[0].x * 1000 + 500;
    int ty = UsablePath[0].y * 1000 + 500;
    ...
    if(Map[tx][ty]!=7U)
    {
        api.Move((int)(sqrt(dx * dx + dy * dy) / api.GetSelfInfo()->speed), atan2(dy, dx));
    }
    else
    {
        api.SkipWindow();
