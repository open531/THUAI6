...
void MoveDest(IStudentAPI& api, std::vector<Node> UsablePath)
{
	...
	int tx, ty;
	if (UsablePath.size() >= 3 && IsValidWithoutWindows(sx / 1000, sy / 1000) && IsValidWithoutWindows(UsablePath[1].x, UsablePath[1].y) && IsValidWithoutWindows(UsablePath[2].x, UsablePath[2].y) && IsValidWithoutWindows(sx / 1000, UsablePath[2].y) && IsValidWithoutWindows(UsablePath[2].x, sy / 1000))
	{
